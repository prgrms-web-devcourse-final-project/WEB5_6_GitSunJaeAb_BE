...
public class RoadmapResource {
    ...
    public RoadmapResource(final RoadmapService roadmapService, final MemberRepository memberRepository,
    ...
    @GetMapping
    public ResponseEntity<List<RoadmapDTO>> getAllMaps() {
        return ResponseEntity.ok(roadmapService.findAll());
    }

    @GetMapping("/{id}")
    public ResponseEntity<RoadmapDTO> getMap(@PathVariable(name = "id") final Long id) {
        return ResponseEntity.ok(roadmapService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<Long> createMap(@RequestBody @Valid final RoadmapDTO roadmapDTO) {
        final Long createdId = roadmapService.create(roadmapDTO);
    ...
    public ResponseEntity<Long> updateMap(@PathVariable(name = "id") final Long id,
        ...
        roadmapService.update(id, roadmapDTO);
    ...
    public ResponseEntity<Void> deleteMap(@PathVariable(name = "id") final Long id) {
        final ReferencedWarning referencedWarning = roadmapService.getReferencedWarning(id);
