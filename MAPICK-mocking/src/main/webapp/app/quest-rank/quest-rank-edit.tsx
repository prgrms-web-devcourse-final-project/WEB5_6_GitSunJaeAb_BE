...
export default function QuestRankEdit() {
  const { t } = useTranslation();
  useDocumentTitle(t('questRank.edit.headline'));
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('questRank.edit.headline')}</h1>
      <div>
        <Link to="/questRanks" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('questRank.edit.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(updateQuestRank)} noValidate>
      <InputRow useFormResult={useFormResult} object="questRank" field="id" disabled={true} type="number" />
      ...
      <input type="submit" value={t('questRank.edit.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 mt-6" />
    </form>
  </>);
}
