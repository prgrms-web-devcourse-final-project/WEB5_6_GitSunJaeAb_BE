...
export default function MapHashtagRelationEdit() {
  const { t } = useTranslation();
  useDocumentTitle(t('mapHashtagRelation.edit.headline'));
  ...
  const prepareForm = async () => {
    try {
      ...
      const data = (await axios.get('/api/mapHashtagRelations/' + currentId)).data;
  ...
  const updateMapHashtagRelation = async (data: MapHashtagRelationDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.put('/api/mapHashtagRelations/' + currentId, data);
      navigate('/mapHashtagRelations', {
            state: {
              msgSuccess: t('mapHashtagRelation.update.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('mapHashtagRelation.edit.headline')}</h1>
      <div>
        <Link to="/mapHashtagRelations" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('mapHashtagRelation.edit.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(updateMapHashtagRelation)} noValidate>
      <InputRow useFormResult={useFormResult} object="mapHashtagRelation" field="id" disabled={true} type="number" />
      ...
      <input type="submit" value={t('mapHashtagRelation.edit.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 mt-6" />
    </form>
  </>);
}
