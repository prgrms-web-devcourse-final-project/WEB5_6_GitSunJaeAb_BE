...
export default function RoadmapHashtagRelationList() {
  ...
  const [roadmapHashtagRelations, setMapHashtagRelations] = useState<MapHashtagRelationDTO[]>([]);
  const navigate = useNavigate();

  const getAllMapHashtagRelations = async () => {
    try {
      const response = await axios.get('/api/roadmapHashtagRelations');
  ...
  const confirmDelete = async (id: number) => {
    ...
    try {
      await axios.delete('/api/roadmapHashtagRelations/' + id);
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      ...
      <div>
        <Link to="/roadmapHashtagRelations/add" className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2">{t('mapHashtagRelation.list.createNew')}</Link>
      </div>
    </div>
    ...
    <div className="overflow-x-auto">
      <table className="w-full">
        ...
        <tbody className="border-t-2 border-black">
          {roadmapHashtagRelations.map((mapHashtagRelation) => (
          <tr key={mapHashtagRelation.id} className="odd:bg-gray-100">
            ...
            <td className="p-2">
              <div className="float-right whitespace-nowrap">
                <Link to={'/roadmapHashtagRelations/edit/' + mapHashtagRelation.id} className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-3 rounded px-2.5 py-1.5 text-sm">{t('mapHashtagRelation.list.edit')}</Link>
                <span> </span>
