...
export default function MemberQuestAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('memberQuest.add.headline'));
  ...
  const createMemberQuest = async (data: MemberQuestDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/memberQuests', data);
      navigate('/memberQuests', {
            state: {
              msgSuccess: t('memberQuest.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('memberQuest.add.headline')}</h1>
      <div>
        <Link to="/memberQuests" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('memberQuest.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createMemberQuest)} noValidate>
      ...
      <input type="submit" value={t('memberQuest.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 mt-6" />
    </form>
  </>);
}
