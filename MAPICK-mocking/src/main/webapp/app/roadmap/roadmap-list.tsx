      ...
      <table className="w-full">
        <thead>
          <tr>
            ...
            <th scope="col" className="text-left p-2">{t('Category')}</th>
            ...
            <th scope="col" className="text-left p-2">{t('Hashtags')}</th>
        ...
        <tbody className="border-t-2 border-black">
          {roadmaps.map((roadmap) => (
              <tr key={roadmap.id} className="odd:bg-gray-100">
                ...
                <td className="p-2">{roadmap.categoryName}</td>
                <td className="p-2">{roadmap.title}</td>
                <td className="p-2">{roadmap.thumbnail}</td>
                <td className="p-2">{roadmap.isPublic?.toString()}</td>
                <td className="p-2">
                  {roadmap.hashtags?.map((hashtag) => hashtag.name).join(', ')}
                </td>
                <td className="p-2">{roadmap.likeCount}</td>
                <td className="p-2">{roadmap.viewCount}</td>
                <td className="p-2">{roadmap.roadmapType?.toString()}</td>
                <td className="p-2">
                  <div className="float-right whitespace-nowrap">
                    <Link to={'/roadmaps/edit/' + roadmap.id}
                          className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-3 rounded px-2.5 py-1.5 text-sm">{t('roadmap.list.edit')}</Link>
                    <span> </span>
                    <button type="button" onClick={() => confirmDelete(roadmap.id!)}
                            className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-3 rounded px-2.5 py-1.5 text-sm">{t('roadmap.list.delete')}</button>
                  </div>
                </td>
              </tr>
          ))}
