...
export default function MapAdd() {
  ...
  useDocumentTitle(t('map.add.headline'));
  ...
  const createMap = async (data: MapDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/maps', data);
      navigate('/maps', {
            state: {
              msgSuccess: t('map.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('map.add.headline')}</h1>
      <div>
        <Link to="/maps" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('map.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createMap)} noValidate>
      ...
      <input type="submit" value={t('map.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 mt-6" />
    </form>
  </>);
}
