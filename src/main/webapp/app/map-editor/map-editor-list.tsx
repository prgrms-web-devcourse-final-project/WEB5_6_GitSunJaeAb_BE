...
export default function MapEditorList() {
  const { t } = useTranslation();
  useDocumentTitle(t('mapEditor.list.headline'));

  const [mapEditors, setMapEditors] = useState<MapEditorDTO[]>([]);
  const navigate = useNavigate();

  const getAllMapEditors = async () => {
    try {
      const response = await axios.get('/api/mapEditors');
  ...
  const confirmDelete = async (id: number) => {
    ...
    try {
      await axios.delete('/api/mapEditors/' + id);
      navigate('/mapEditors', {
            state: {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      ...
      <div>
        <Link to="/mapEditors/add" className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2">{t('mapEditor.list.createNew')}</Link>
      </div>
    </div>
    {!mapEditors || mapEditors.length === 0 ? (
          ...
          <tr key={mapEditor.id} className="odd:bg-gray-100">
            ...
            <td className="p-2">
              <div className="float-right whitespace-nowrap">
                <Link to={'/mapEditors/edit/' + mapEditor.id} className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-3 rounded px-2.5 py-1.5 text-sm">{t('mapEditor.list.edit')}</Link>
                <span> </span>
