...
export default function LayerAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('layer.add.headline'));
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('layer.add.headline')}</h1>
      <div>
        <Link to="/layers" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('layer.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createLayer)} noValidate>
